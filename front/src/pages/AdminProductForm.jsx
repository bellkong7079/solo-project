...
function AdminProductForm() {
  ...
  const fetchCategories = async () => {
    try {
      const token = localStorage.getItem('adminToken');
      const response = await axios.get('http://192.168.0.225:5000/api/admin/categories', {
  ...
  const fetchProductData = async () => {
    try {
      const token = localStorage.getItem('adminToken');
      const response = await axios.get(`http://192.168.0.225:5000/api/products/${id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.put(`http://192.168.0.225:5000/api/admin/products/${id}`, formDataToSend, {
        headers: {
              ...
              <div className="image-preview-container">
                {existingImages.map((img, index) => (
                  <div key={img.image_id} className="image-preview">
                    <img src={`http://192.168.0.225:5000${img.image_url}`} alt={`기존 이미지 ${index + 1}`} />
